	...
	public String Test(String key , int page) {
		...
		try {
			 StringBuilder urlBuilder = new StringBuilder("http://apis.data.go.kr/B552657/HsptlAsembySearchService/getHsptlMdcncListInfoInqire"); /*URL*/
			  ...
			  urlBuilder.append("&" + URLEncoder.encode("Q0","UTF-8") + "=" + URLEncoder.encode("서울특별시", "UTF-8")); /*주소(시군구)*/
			  urlBuilder.append("&" + URLEncoder.encode("Q1","UTF-8") + "=" + URLEncoder.encode("마포구", "UTF-8")); /*주소(시군구)*/
			  urlBuilder.append("&" + URLEncoder.encode("Q2","UTF-8") + "=" + URLEncoder.encode(""+key, "UTF-8")); //q1만 쓸시 너무 많이나옴
